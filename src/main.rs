// #![warn(rust_2018_idioms)]
// // If no backend is enabled, a large portion of the codebase is unused.
// // So silence this useless warning for the CI.
// #![cfg_attr(
//     not(any(feature = "winit", feature = "udev")),
//     allow(dead_code, unused_imports)
// )]

mod backend;
#[cfg(any(feature = "udev", feature = "xwayland"))]
mod cursor;
mod drawing;
mod focus;
mod input;
mod render;
mod shell;
mod state;

use state::{AnvilState, ClientState};

static POSSIBLE_BACKENDS: &[&str] = &[
    #[cfg(feature = "winit")]
    "--winit : Run kawm as a X11 or Wayland client using winit.",
    #[cfg(feature = "udev")]
    "--tty-udev : Run kawm as a tty udev client (requires root if without logind).",
];

#[cfg(debug_assertions)]
#[cfg(feature = "profile-with-tracy-mem")]
#[global_allocator]
static GLOBAL: profiling::tracy_client::ProfiledAllocator<std::alloc::System> =
    profiling::tracy_client::ProfiledAllocator::new(std::alloc::System, 10);

fn main() {
    if let Ok(env_filter) = tracing_subscriber::EnvFilter::try_from_default_env() {
        tracing_subscriber::fmt()
            .compact()
            .with_env_filter(env_filter)
            .init();
    } else {
        tracing_subscriber::fmt().compact().init();
    }

    #[cfg(feature = "profile-with-tracy")]
    profiling::tracy_client::Client::start();

    profiling::register_thread!("Main Thread");

    #[cfg(feature = "profile-with-puffin")]
    let _server =
        puffin_http::Server::new(&format!("0.0.0.0:{}", puffin_http::DEFAULT_PORT)).unwrap();
    #[cfg(feature = "profile-with-puffin")]
    profiling::puffin::set_scopes_on(true);

    let arg = ::std::env::args().nth(1);
    match arg.as_ref().map(|s| &s[..]) {
        #[cfg(feature = "winit")]
        Some("--winit") => {
            tracing::info!("Starting kawm with winit backend");
            backend::winit::run_winit();
        }
        #[cfg(feature = "udev")]
        Some("--tty-udev") => {
            tracing::info!("Starting kawm on a tty using udev");
            backend::udev::run_udev();
        }
        Some(other) => {
            tracing::error!("Unknown backend: {}", other);
        }
        None => {
            #[allow(clippy::disallowed_macros)]
            {
                println!("USAGE: kawm --backend");
                println!();
                println!("Possible backends are:");
                for b in POSSIBLE_BACKENDS {
                    println!("\t{}", b);
                }
            }
        }
    }
}
